...
contract ERC721StakingTest is Test {
    ...
    function test_Simulation5() public {
        /// Simulation: owner initializes the staking contract and load rewards. At startAt actor 1 stakes 5 NFTs. After 10 seconds the owner shrink the duration so that the rewardRate increases. After 10 seconds the user claims his rewards.
        ...
        // owner shrinks duration doubling the reward rate
        uint256 previousRewardRate = staking.rewardRate();
        uint256 newEndAt = block.timestamp + (endAt - block.timestamp) / 2;
        vm.prank(owner);
        staking.setEndAt(newEndAt);

        // evaluate
        assertEq(staking.endAt(), newEndAt);
        assertEq(staking.rewardRate(), previousRewardRate * 2);
        assertEq(staking.rewardPerTokenStored(), 10 * 30 * 1e18 / 5);
        assertEq(staking.lastUpdateTime(), startAt + 10);
        assertEq(staking.userRewardPerTokenPaid(actor1), 0);
        assertEq(staking.totalStaked(), stakeAmount);
        assertEq(staking.toDistributeRewards(), initialRewardAllocated - 300);
        ...
        // no left over
        uint256 leftover = staking.toDistributeRewards() - (staking.endAt() - block.timestamp) * staking.rewardRate();
        assertEq(leftover, 0);
        ...
        // evaluate
        assertEq(collection.balanceOf(actor1), stakeAmount);
        assertEq(rewardToken.balanceOf(actor1), 900);
        assertEq(staking.rewardPerTokenStored(), (10 * 30 * 1e18 / 5) + (10 * 60 * 1e18 / 5));
        assertEq(staking.lastUpdateTime(), startAt + 20);
        assertEq(staking.userRewardPerTokenPaid(actor1), (10 * 30 * 1e18 / 5) + (10 * 60 * 1e18 / 5));
        assertEq(staking.totalStaked(), 0);
        assertEq(staking.toDistributeRewards(), initialRewardAllocated - 900);
        assertEq(staking.owedRewards(), 0);
    }

    function test_Simulation6() public {
        /// Simulation: owner initializes the staking contract and load rewards. At startAt actor1 stakes 5 NFTs. After 10 seconds the owner add more token to the contract so that the rewardRate increases. After 10 seconds the user claims his rewards.
        ...
        // owner adds more tokens to the contract
        uint256 additionalReward = 6000;
        uint256 newRewardRate =
            (initialRewardAllocated + additionalReward - staking.earned(actor1)) / (endAt - block.timestamp);
        ...
        assertEq(staking.toDistributeRewards(), initialRewardAllocated + additionalReward - 300);
        assertEq(staking.owedRewards(), 300);
    ...
    function test_Simulation8() public {
        /// Simulation: owner initializes the staking contract and loads rewards. At time startAt the actor1 stakes 5 NFTs. After 10 seconds the actor2 stakes 5 NFTs. After 10 seconds the user3 stakes 5 NFTs. After 10 seconds the owner remove some rewards from the contract so that the rewardRate decreases. After 10 seconds the user1 claims rewards. After 10 seconds the user2 claims rewards. After 10 seconds the user3 claims rewards. The owner recover the excess reward from the contract.
        ...
        assertEq(staking.rewardRate(), 30);
        assertEq(staking.toDistributeRewards(), initialRewardAllocated - 300);
        assertEq(staking.rewardPerToken(), 10 * 30 * 1e18 / 5);
        assertEq(staking.userRewardPerTokenPaid(actor2), 10 * 30 * 1e18 / 5);
        assertEq(staking.lastUpdateTime(), startAt + 10);
        ...
        assertEq(staking.earned(actor3), 0);
        assertEq(staking.rewardRate(), 30);
        assertEq(staking.toDistributeRewards(), initialRewardAllocated - 600);
        assertEq(staking.rewardPerToken(), (10 * 30 * 1e18 / 5) + (10 * 30 * 1e18 / 10));
        assertEq(staking.userRewardPerTokenPaid(actor3), (10 * 30 * 1e18 / 5) + (10 * 30 * 1e18 / 10));
        assertEq(staking.lastUpdateTime(), startAt + 20);

        // evaluate actor2 and actor1 position
        assertEq(staking.earned(actor2), 150);
        assertEq(staking.earned(actor1), 450);
        ...
        // owner remove some rewards
        uint256 removedReward = 6000;
        uint256 newRewardRate = (
            initialRewardAllocated - removedReward - staking.earned(actor1) - staking.earned(actor2)
                - staking.earned(actor3)
        ...
        // evaluate
        assertEq(staking.rewardRate(), newRewardRate);
        assertEq(staking.toDistributeRewards(), initialRewardAllocated - 900 - removedReward);
        assertEq(staking.rewardPerToken(), (10 * 30 * 1e18 / 5) + (10 * 30 * 1e18 / 10) + (10 * 30 * 1e18 / 15));
        ...
        // evaluate actor2 position
        assertEq(collection.balanceOf(actor2), 5);
        assertEq(rewardToken.balanceOf(actor2), 250 + (newRewardRate * 10) / 3 + (newRewardRate * 10) / 2);

        // evaluate actor3 position
        assertEq(staking.earned(actor3), 100 + (newRewardRate * 10) / 3 + (newRewardRate * 10) / 2);
        ...
        assertEq(
            ...
            100 + (newRewardRate * 10) / 3 + (newRewardRate * 10) / 2 + (newRewardRate * 10) / 1
        );

        // evaluate staking contract
        assertEq(staking.totalStaked(), 0);
        assertEq(
            staking.toDistributeRewards(),
            initialRewardAllocated - removedReward - (30 * 10 * 3) - (newRewardRate * 10 * 3)
        );
